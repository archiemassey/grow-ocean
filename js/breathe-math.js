...
/* Where are we in one breath cycle at time t (seconds), given the three phase
   lengths? Returns { phase: 'in' | 'hold' | 'out', p: 0..1 progress in phase }.
   A zero-length phase is skipped (reported complete). */
export function cyclePhase(t, inS, holdS, outS) {
  const total = Math.max(0.001, inS + holdS + outS);
  let tt = t % total;
  if (tt < 0) tt += total;
  if (tt < inS) return { phase: 'in', p: inS > 0 ? tt / inS : 1 };
  if (tt < inS + holdS) return { phase: 'hold', p: holdS > 0 ? (tt - inS) / holdS : 1 };
  return { phase: 'out', p: outS > 0 ? (tt - inS - holdS) / outS : 1 };
}
